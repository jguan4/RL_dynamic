...
class AcrobotEnv_mag(core.Env):
    ...
    **REFERENCE:**
    ...
    def __init__(self):
        ...
        self.AVAIL_TORQUE = np.divide(self.AVAIL_TORQUE, self.factor)
        self.AVAIL_TORQUE = self.AVAIL_TORQUE*self.mag
        self.seed()
    ...
    def reset(self):
        ...
        self.state[0] = self.np_random.uniform(low=-np.pi,high=-np.pi,size=(1,))
        self.state[1:4] = self.np_random.uniform(low=0.,high=0.,size=(3,))
    ...
    def _terminal(self):
        s = self.state
        return bool(-np.cos(s[0]) - np.cos(s[1] + s[0]) > 2.)
